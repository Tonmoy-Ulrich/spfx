...
import styles from "./ReactCrudPnP.module.scss";
import { IReactCrudPnPProps } from "./IReactCrudPnPProps";
...
export default function ReactCrudPnP(props: IReactCrudPnPProps): JSX.Element {
    ...
    const getAllItems = async () => {
        try {
            const items: any[] = await sp.web.lists.getByTitle(props.listName).items();
            // console.log(items);

            if (items.length > 0) {
                const jobItems: any = await sp.web.lists.getByTitle("Job Title Lists").items();
                const deptItems: any = await sp.web.lists.getByTitle("Department Lists").items();
    ...
    const createItem = async () => {
        try {
            const addItem = await sp.web.lists.getByTitle(props.listName).items.add({
    ...
    const getItemById = async (id) => {
        try {
            const item: any = await sp.web.lists.getByTitle(props.listName).items.getById(id);
            // console.log(item);
        } catch (e) {
            console.error(e);
        }
    ...
    //Update Item
    const updateItem = async (employeeInfo) => {
        try {
    ...
    const deleteItem = async (id) => {
        if (!window.confirm(`Are you sure you want to delete the item with ${id}?`)) {
            return;
        }

        try {
            await sp.web.lists.getByTitle(props.listName).items.getById(id).delete();
            await getAllItems();
