        ...
        if (!this._topPlaceholder) {
            ...
            if (this.properties) {
                ...
                if (this._notAcceptedTasks.length > 0) {
                    topString = `${escape(
                        "You have New project assigned which need to be accepted",
                    )}&nbsp;&nbsp;
                        <a href="https://nstechltd.sharepoint.com/sites/SharePointRnD/Lists/Project%20Lists/AllItems.aspx">${escape(
                            "GoTo List",
                        )}</a>`;
                }

                if (!topString) {
                    topString = `${escape("Top property was not defined.")}`;
                }
